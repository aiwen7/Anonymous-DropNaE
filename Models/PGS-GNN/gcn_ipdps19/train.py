...
flags.DEFINE_boolean('cpu_eval',True,'whether to use cpu to eval')
...
flags.DEFINE_boolean('use_DropNaE',None,'whether to use DropNaE')
...
def train(train_phases,train_params,dims_gcn,model,minibatch,\
    ...
    for ip,phase in enumerate(train_phases):
        ...
        for e in range(epoch_ph_start,phase['end']):
            ...
            while not minibatch.end():
                ...
                sample_start = ttime.time()
                feed_dict, labels = minibatch.minibatch_train_feed_dict(phase['dropout'],is_val=False,is_test=False)
                sample_time += ttime.time() - sample_start
                
                t0=ttime.time()
                _,__,loss_train,pred_train = sess.run([train_stat[0], model.opt_op, model.loss, model.preds], feed_dict=feed_dict)
                t1 = ttime.time()
                pure_train_time += t1 - t0
                time_epoch += t1-t0
                if not minibatch.batch_num % FLAGS.eval_train_every:
                    calc_start = ttime.time()
                    ...
                    calc_f1_time += ttime.time() - calc_start
            ...
            if FLAGS.cpu_eval:      # Full batch evaluation using CPU
                ...
                sess_eval=sess
            if e%FLAGS.eval_train_every:
                eval_start = ttime.time()
                loss_val,f1mic_val,f1mac_val = evaluate_full_batch(sess_eval,model,minibatch,mode='val')
                eval_time += ttime.time() - eval_start
            ...
            printf('  samping time  {:.5f}'.format(sample_time))
    ...
    printf("Total train time : {:.4f}".format(ttime.time()- total_train_start),style='red')
    printf("Sampling time : {:.4f}".format(sample_time),style='red')
    printf("Pure_train_time : {:.4f}".format(pure_train_time),style='red')
    printf("Eval_time : {:.4f}".format(eval_time),style='red')
    printf("Calc_f1_time : {:.4f}".format(calc_f1_time),style='red')
    printf("Optimization Finished!",style='red')
    loss, f1_mic, f1_mac = evaluate_full_batch(sess_eval,model,minibatch,mode='val')
    printf("Full validation stats: loss={:.5f}, f1_micro={:.5f}, f1_macro={:.5f}".format(loss,f1_mic,f1_mac),style='yellow')
    loss, f1_mic, f1_mac = evaluate_full_batch(sess_eval,model,minibatch,mode='test')
