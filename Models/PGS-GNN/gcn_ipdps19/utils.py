...
def load_data(prefix, dropNaE, normalize=True):
    adj_full = scipy.sparse.load_npz('./{}/adj_full.npz'.format(prefix))
    
    if dropNaE == 'N':
        print("use DropNaE-N!")
        adj_train = scipy.sparse.load_npz('./{}/adj_train_DR_pg.npz'.format(prefix))
    elif dropNaE == 'E':
        print("use DropNaE-E!")
        adj_train = scipy.sparse.load_npz('./{}/adj_train_DE_pg.npz'.format(prefix))
    else:
        print("No DropNaE!")
...
def parse_n_prepare(flags):
    ...
    temp_data = load_data(flags.data_prefix,dropreef=flags.use_DropReef)
