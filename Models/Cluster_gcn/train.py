...
def load_data(data_prefix, dataset_str, precalc,DropNaE):
  """Return the required data formats for GCN models."""
  (num_data, train_adj, full_adj, feats, train_feats, test_feats, labels,
   train_data, val_data,
   test_data) = utils.load_graphsage_data(data_prefix, dataset_str, DropNaE = DropNaE)
...
def main(unused_argv):
  ...
  (train_adj, full_adj, train_feats, test_feats, y_train, y_val, y_test,
   ...
   visible_data) = load_data(FLAGS.data_prefix, FLAGS.dataset, FLAGS.precalc,FLAGS.use_DropNaE)

  # Partition graph and do preprocessing
  if FLAGS.bsize > 1:
    _, parts, train_cluster_time= partition_utils.partition_graph(train_adj, visible_data,
                                               FLAGS.num_clusters)
    parts = [np.array(pt) for pt in parts]
  else:
    (parts, features_batches, support_batches, y_train_batches,
     train_mask_batches), train_cluster_time= utils.preprocess(train_adj, train_feats, y_train,
                                            train_mask, visible_data,
                                            FLAGS.num_clusters,
                                            FLAGS.diag_lambda)

  (_, val_features_batches, val_support_batches, y_val_batches,
   val_mask_batches),_ = utils.preprocess(full_adj, test_feats, y_val, val_mask,
                                        np.arange(num_data),
                                        FLAGS.num_clusters_val,
                                        FLAGS.diag_lambda)

  (_, test_features_batches, test_support_batches, y_test_batches,
   test_mask_batches),_ = utils.preprocess(full_adj, test_feats, y_test,
  ...
  with tf.device('/cpu:0'):
    ...
    print("\033[93m Total train time:{:.2f} \033[0m".format(pure_train_time + train_cluster_time))
    tf.logging.info(print_str)
